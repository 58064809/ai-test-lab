...
def test_orchestrator_dry_run_generates_plan_and_writes_internal_memory(tmp_path: Path) -> None:
    ...
    result = orchestrator.run("根据这个需求生成测试用例", dry_run=True)
...
def test_orchestrator_ambiguous_input_requires_confirmation(tmp_path: Path) -> None:
    ...
    result = orchestrator.run("帮我看看这个", dry_run=True)
...
def test_orchestrator_preserves_non_execution_boundary_in_non_dry_run_mode(tmp_path: Path) -> None:
    ...
    result = orchestrator.run("请修改文件并修复路径引用", dry_run=False)
