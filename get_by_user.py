...
import requests, pendulum, pathlib, pymysql, time
...
class Connection:
    ...
    @classmethod
    async def init_pool(cls, loop):
        try:
            # log.info('连接mysql')
            __pool = await aiomysql.create_pool(

                minsize=10,  # 初始化时,连接池至少创建的空闲连接,0表示不创建
                host="pc-2zefs39o1el89k1jn.rwlb.rds.aliyuncs.com",
                port=3306,
                user="ll_db_tag",
                password="GE4FGaZWDKY2kL",
                db="posttag",
                autocommit=True,
                # charset=conf.get_config_value(config_key, 'charset'),
                loop=loop
            )
            return __pool
    ...
    @classmethod
    async def get_async_pool(cls, loop):
        async_pool = await cls.init_pool(loop)
        cls.async_pool = async_pool
        return cls
...
class WeiXin:
    ...
    def __init__(self):
        self.corpid = "wwa65d1e1d596850ef"
        self.contacts_token = "nQMw8d-d6sx1Qgdulg9rvnVcf0QnLpIGNz27fB7mrw8"
        self.customer_token = "P3PG-YObOUXU1VjoYP_4I_pPzrymdt6kuJqhJCZzOeY"
        self.contacts_token_path = pathlib.Path('contacts_token.txt')
        self.customer_token_path = pathlib.Path('customer_token.txt')
    ...
    def get_access_token(self, business):
        if business == "contacts":
            corpsecret = self.contacts_token
            path = self.contacts_token_path
        else:
            corpsecret = self.customer_token
            path = self.customer_token_path
    ...
    def get_users(self):
        ...
        if resp['errcode'] == 0:
            ...
            try:
                sql = 'truncate table `posttag`.`ads_wework_userid_list`'
    ...
    async def insert_userid_list(self, result_list, mysqlobj):
        for item in result_list:
            try:
                sql = "INSERT INTO `posttag`.`ads_wework_userid_list`(unionid,userid,remark,description,createtime,tag_id,remark_mobiles,add_way,oper_userid,external_userid,name,type,avatar,gender,agent) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
                ...
                id = await mysqlobj.async_commit_oper(sql, (
                    item['external_contact']['unionid'], item['follow_info']['userid'],
    ...
    async def main(self):
        ...
        mysqlobj = await Connection.get_async_pool(loop)
        total = self.get_users()
