...
class eCommerce:
    ...
    def get_users(self):
        ...
        if resp['errcode'] == 0:
            ...
            try:
                sql = 'truncate table `posttag`.`ads_wework_eCommerce_userid_list`'
    ...
    async def insert_userid_list(self, result_list, mysqlobj):
        for item in result_list:
            try:
                sql = "INSERT INTO `posttag`.`ads_wework_eCommerce_userid_list`(unionid,userid,remark,description,createtime,tag_id,remark_mobiles,add_way,oper_userid,external_userid,name,type,avatar,gender,agent) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
